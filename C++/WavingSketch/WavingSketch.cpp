...
unsigned int ReadInTraces(const char* traceFilePath, const char* outputFilePath, char** keys) {
    FILE* fin = fopen(traceFilePath, "rb");
    FILE* fout = fopen(outputFilePath, "wb");  // 打开输出文件
    ...
    while (fread(temp, 1, KEY_SIZE, fin) == KEY_SIZE) { // 读取源 IP
        key = new char[KEY_SIZE]{ 0 };
        memcpy(key, temp, KEY_SIZE);
        keys[count] = key;
...
int main()
{
    ...
    // unsigned int actualItemNum = ReadInTraces(R"(../data/20190117-130000-new.dat)", R"(../data/20190117-130000-new-sample.dat)", keys);
